// Final2Fwmn.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
...
int main(int argc, const char* argv[]) {
	...
	for (int numthreads_2 = 12; numthreads_2 <= 12; numthreads_2++) {
		cout << "star with " << numthreads_2 << " cpus N=" << N << ", D=" << sizeD << " parallel of N= " << N << endl;
		tomp1 = omp_get_wtime();
		...
		#pragma omp parallel for num_threads(numthreads_2) schedule(static) shared(Lmax,Ld,Po,cl)
		...
		cout << "N =" << N << "threads level one " << 1 << "-" << " threads level inside " << numthreads_2 << " " << " duration= " << chrono::duration_cast<chrono::milliseconds>(elapsed).count() << " ms" << endl;

		times << "{ " << 1 << ", " << numthreads_2 << ", " << chrono::duration_cast<chrono::milliseconds>(elapsed).count() << "}," << endl;
		tomp2 = omp_get_wtime();
		double dt = tomp2 - tomp1;
		double res = 1.0 / omp_get_wtick();
		cout << "elapse time " << dt << " seg" << endl;
		cout << "clock resolution " << res << endl;
